...
const emptyForm = { title: "", description: "", img: "", url_website: "", url_github: "" };
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-white/40 text-sm mt-1">Kelola project yang ditampilkan di portofolio</p>
      </div>
      ...
      <div className="p-6 rounded-2xl bg-white/5 border border-white/10 mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <div
              ...
              className="relative flex items-center gap-3 px-4 py-3 rounded-xl bg-white/5 border border-dashed border-white/20 hover:border-[#F1FF5E]/50 hover:bg-white/8 transition-all cursor-pointer group"
            >
              {preview ? (
                <>
                ...
                <>
                  <div className="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center text-xl shrink-0">🖼️</div>
                  <div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              value={form.img}
              onChange={(e) => { setForm({ ...form, img: e.target.value }); setPreview(e.target.value); }}
              className="px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-[#F1FF5E]/50 transition-all"
            />
          </div>
        ...
        {preview && (
          <div className="mt-4">
            ...
            <img src={preview} alt="preview" className="w-full max-w-sm h-40 object-cover rounded-xl border border-white/10" />
          </div>
        )}
      ...
      <div className="flex flex-col gap-3">
        {projects.length === 0 && (
          <div className="text-center py-12 text-white/30 text-sm">Belum ada project. Tambahkan sekarang!</div>
        )}
        {projects.map((p) => (
          <div key={p.id} className="flex items-center gap-4 p-4 rounded-xl bg-white/5 border border-white/10 hover:border-white/20 transition-all">
            {p.img ? (
              <img src={p.img} alt={p.title} className="w-16 h-16 rounded-xl object-cover shrink-0 bg-white/10" />
            ) : (
              <div className="w-16 h-16 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-2xl shrink-0">🖼️</div>
            )}
            <div className="flex-1 min-w-0">
              <p className="font-semibold truncate">{p.title}</p>
              <p className="text-white/40 text-sm truncate">{p.description}</p>
